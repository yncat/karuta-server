...
function processRemoteControlRequest(sender_connection) {
  if (remote_controller) {
    sendMessageTo(
      { command: "remote_control_request_result", result: "false" },
      sender_connection
    );
  } else {
    remote_controller = sender_connection;
    sendMessageTo(
      { command: "remote_control_request_result", result: "true" },
      sender_connection
    );
  }
}
...
function processChallengeRequest(m) {
  ...
  sound_candidates = sound_candidates.filter(elem => {
    return elem != right_sound;
  });
  const sound_candidates_backup = sound_candidates.slice(
    0,
    sound_candidates.length
  );
  ...
  for (var i = 0; i < cards.length; i++) {
    ...
    if (sound_candidates.length > 0) {
      ...
      play_list[i] =
        sound_candidates_backup[
          Math.floor(Math.random() * sound_candidates_backup.length)
        ];
    }
  }
